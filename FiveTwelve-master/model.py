...
class Grid(Game_Element):
    ...
    def left(self):
        ...
        ## only reverse oce because it searches left-> right anyway
    ...
    def right(self):
        ...
        ## double reverse so that it catches it on the right
        for col in reversed(self.tiles):
    ...
    def up(self):
        """Slide tiles up"""
        ## Check normally all the way through
        for row in self.tiles:
            for col in row:
                if col:
                    col.slide(self,(-1,0))
    ...
    def down(self):
        """Slide tiles down"""
        ## reverse once so that you can check the tiles up to down
